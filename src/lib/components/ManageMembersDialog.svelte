...
<Dialog.Root bind:open>
	<Dialog.Portal>
		...
		>
			<Dialog.Title class="text-base font-semibold text-gray-900">Manage members</Dialog.Title>

			{#if error}
				<p class="mt-2 text-xs text-red-600" role="alert">{error}</p>
			{/if}

			<h4 class="mt-4 text-xs font-medium text-gray-700">
			...
			>
				{#each chat.members as member (member.xmppUsername)}
					...
					<li class="flex items-center gap-2.5 px-3 py-2 text-sm">
						...
						{/if}
						<span class="min-w-0 flex-1 truncate text-gray-900">{memberName(member)}</span>
						...
						{#if !mine}
							<button
								...
								disabled={busy}
								class="shrink-0 rounded-md bg-white px-2 py-0.5 text-xs font-medium text-red-600 ring-1 ring-inset ring-red-200 hover:bg-red-50 disabled:opacity-50"
							>
			...
			<fieldset class="mt-4 min-w-0">
				<legend class="text-xs font-medium text-gray-700">Add people</legend>
				...
				{#if memberTab === 'all'}
					<input
						...
						class="mt-1.5 block w-full rounded-md border-0 px-3 py-1.5 text-sm text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600"
					/>
				{/if}
			</fieldset>
